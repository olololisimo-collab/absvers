...
export default function AdminOrdersPanel() {
        ...
        if (stored) {
          ...
          if (Array.isArray(parsed) && parsed.length > 0) {
            ...
            const normalized: OrderItem[] = parsed.map((ord: any) => ({
              ...
              emailSentAt: ord.emailSentAt || (ord.status === "kp_sent" || ord.status === "paid" ? "15.09.2026 12:30" : undefined),
              emailStatus: ord.emailStatus || (ord.status === "kp_sent" || ord.status === "paid" ? "sent" : "not_sent"),
            }));
  ...
  return (
    <AdminLoginGate
      title="Заявки и КП — Доступ администратора"
      description="Для просмотра поступивших смет, контактов клиентов и выставления счетов введите PIN-код или учетные данные."
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-3xl shadow-sm border border-slate-200 mb-6">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          <button 
            ...
            className="px-3.5 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-bold flex items-center gap-1.5 transition cursor-pointer border border-slate-200"
            title="Смена логина, пароля и PIN-кода"
          >
            <Lock className="w-4 h-4 text-slate-600" />
            <span>Пароли и доступ</span>
          </button>

          <button 
            onClick={() => alert("Экспорт всех заявок в Excel (.XLSX) сформирован")}
            className="px-3.5 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-semibold flex items-center gap-2 transition cursor-pointer"
          >
          ...
          <button
            ...
            title="Заблокировать и выйти из админ-панели"
          >
                  ...
                  return (
                    <tr key={ord.id} className="hover:bg-slate-50/80 transition group">
                      ...
                      <td className="p-4">
                        ...
                        {/* Email status badge in table */}
                        <div className="mt-1 flex items-center gap-1 text-[10px]">
                          {ord.emailStatus === "sent" ? (
                            <span className="inline-flex items-center gap-1 text-emerald-700 font-semibold bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-200">
                              <Mail className="w-2.5 h-2.5 text-emerald-600" /> Email отправлен
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1 text-slate-400 bg-slate-50 px-1.5 py-0.5 rounded border border-slate-200">
                              <Mail className="w-2.5 h-2.5" /> Email не выслан
                            </span>
                          )}
                        </div>
                      </td>
          ...
          <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-slate-200 p-6 md:p-8 overflow-y-auto max-h-[90vh]">
            ...
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 mb-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                ...
                <div>
                  <span className="text-slate-400 block">Статус Email:</span>
                  {selectedOrder.emailStatus === "sent" ? (
                    <span className="inline-flex items-center gap-1 text-emerald-700 font-bold bg-emerald-50 px-2 py-0.5 rounded">
                      <CheckCircle2 className="w-3 h-3 text-emerald-600" /> Отправлен {selectedOrder.emailSentAt ? `(${selectedOrder.emailSentAt})` : ""}
                    </span>
                    ...
                    <span className="inline-flex items-center gap-1 text-rose-700 font-bold bg-rose-50 px-2 py-0.5 rounded">
                      <AlertCircle className="w-3 h-3 text-rose-600" /> Ошибка отправки
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1 text-slate-500 font-medium bg-slate-100 px-2 py-0.5 rounded">
                      <Clock className="w-3 h-3 text-slate-400" /> Не отправлялся
                    </span>
                  )}
                </div>
              </div>
                      ...
                      setOrders((prev) =>
                        prev.map((o) =>
                          o.id === selectedOrder.id
                            ? { ...o, emailStatus: "sent", emailSentAt: nowStr }
                            : o
                        )
                      );
                      setSelectedOrder((prev) =>
                        prev ? { ...prev, emailStatus: "sent", emailSentAt: nowStr } : null
                      );
